<template>
    <div>
        ...
        <el-col :span="24" class="toolbar">
            <el-button type="danger" @click="atchRemove" :disabled="this.sels.length===0">批量删除</el-button>
            <el-upload
                class="upload-demo"
...
export default {
    ...
    methods: {
        searchByName(page) {
            ...
            this.axios
                .get(httpURL + "token/registration/get_all_registration", {
                    params: {
                        name: name,
                        page: page,
                        size: 10
                    ...
                    this.$confirm("确认提交吗？", "提示", {}).then(() => {
                        if (
                            ...
                            this.axios
                                .put(
                                    httpURL +
                                        "token/registration/update_registration",
                                    this.editForm
                                )
                                .then(res => {
                                    if (this.intercept(res.data)) {
            ...
            this.$confirm("确认删除该记录吗?", "提示", {
                ...
                .then(() => {
                    this.axios
                        .delete(
                            httpURL +
                                "token/registration/delete_registration_by_id/" +
                                row.id
                        )
                        .then(res => {
                            if (this.intercept(res.data)) {
            ...
            this.$confirm("确认删除选中记录吗？", "提示", {
                ...
                .then(() => {
                    ...
                    this.axios
                        .delete(
                            httpURL +
                                "token/registration/delete_many_registration_by_id",
                            {
                                params: {
                                    idStr: ids
                                }
        ...
        uploadSectionFile(e) {
            ...
            this.axios
                .post(
                    httpURL + "token/excel/import_registration",
                    param,
                    config
                )
                .then(res => {
        ...
        download() {
            let token = getToken();
            window.location.href = `${httpURL}token/excel/export_registration?token=${token}`;
        }
    },
    created() {},
    mounted() {
