<template>
    <div>
        ...
        >
            ...
            <el-table-column prop="sex" label="性别" align="center" :formatter="formatSex" width="60"></el-table-column>
            <el-table-column prop="school" label="所在学校" align="center"></el-table-column>
            <el-table-column prop="hostedForm" label="托管形式" align="center" width="90"></el-table-column>
            <el-table-column prop="trainingSubject" label="培训科目" align="center"></el-table-column>
            <el-table-column prop="gradeString" label="成绩信息" align="center" width="300"></el-table-column>
...
export default {
    ...
    methods: {
        searchByName(page) {
            ...
            this.axios
                .get(httpURL + "token/grade/get_all_grade", {
                    params: {
                        name: name,
                        page: page,
                        size: 10
        ...
        atchRemove: function() {
            ...
            this.$confirm("确认删除选中记录吗？", "提示", {
                ...
                .then(() => {
                    ...
                    this.axios
                        .delete(
                            httpURL + "token/grade/delete_many_grade_by_id",
                            {
                                params: {
                                    idStr: ids
                                }
                ...
                if (valid) {
                    this.$confirm("确认提交吗？", "提示", {}).then(() => {
                        if (
                            ...
                            this.axios
                                .put(
                                    httpURL + "token/grade/update_fee",
                                    this.editForm
                                )
                                .then(res => {
                                    if (this.intercept(res.data)) {
        ...
        handleDel(index, row) {
            this.$confirm("确认删除该记录吗?", "提示", {
                ...
                .then(() => {
                    this.axios
                        .delete(
                            httpURL + "token/grade/delete_grade_by_id/" + row.id
                        )
                        .then(res => {
                            if (this.intercept(res.data)) {
                                this.$message({
        ...
        uploadSectionFile(e) {
            ...
            this.load = true;
            this.axios
                .post(httpURL + "token/excel/import_grade", param, config)
                .then(res => {
                    if (this.intercept(res.data)) {
                        // console.log("success");
                        this.upload = false;
        ...
        download() {
            let token = getToken();
            window.location.href = `${httpURL}token/excel/export_grade?token=${token}`;
        }
    },
    created() {},
    mounted() {
