<template>
    <div>
        ...
        >
            ...
            <el-table-column prop="feePayable" label="应缴纳费用" align="center"></el-table-column>
            <el-table-column prop="feePay" label="已缴纳费用" align="center"></el-table-column>
            <el-table-column prop="feeSpread" label="还需缴纳费用" align="center"></el-table-column>
        ...
        <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
            <el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
                ...
                <el-form-item label="应缴纳费用" prop="feePayable">
                    <el-input-number :min="0" v-model="editForm.feePayable" @change="feeSpread"></el-input-number>
                </el-form-item>
                <el-form-item label="已缴纳费用" prop="feePay">
                    <el-input-number
                        :min="0"
                        :max="editForm.feePayable"
                        v-model="editForm.feePay"
                        @change="feeSpread"
                    ></el-input-number>
                </el-form-item>
                <el-form-item label="还需缴纳费用" prop="feeSpread">
                    <el-input-number :min="0" v-model="editForm.feeSpread" disabled></el-input-number>
                </el-form-item>
            </el-form>
...
export default {
    ...
    methods: {
        searchByName(page) {
            ...
            this.axios
                .get(httpURL + "token/fee/get_all_fee", {
                    params: {
                        name: name,
                        page: page,
                        size: 10
        ...
        atchRemove: function() {
            ...
            this.$confirm("确认删除选中记录吗？", "提示", {
                ...
                .then(() => {
                    ...
                    this.axios
                        .delete(httpURL + "token/fee/delete_many_fee_by_id", {
                            params: {
                                idStr: ids
                            }
                        })
        ...
        handleDel(index, row) {
            this.$confirm("确认删除该记录吗?", "提示", {
                ...
                .then(() => {
                    this.axios
                        .delete(
                            httpURL + "token/fee/delete_fee_by_id/" + row.id
                        )
                        .then(res => {
                            if (this.intercept(res.data)) {
                                this.$message({
        ...
        feeSpread() {
            this.editForm.feeSpread =
                this.editForm.feePayable - this.editForm.feePay;
        },
        uploadSectionFile(e) {
            let file = e.file;
            ...
            this.load = true;
            this.axios
                .post(httpURL + "token/excel/import_fee", param, config)
                .then(res => {
                    if (this.intercept(res.data)) {
                        // console.log("success");
                        this.upload = false;
        ...
        download() {
            let token = getToken();
            window.location.href = `${httpURL}token/excel/export_fee?token=${token}`;
        }
    },
    created() {},
    mounted() {
